...
@jwt_required()  # Protege a rota, exigindo um JWT válido
def listar_pacientes():
    # Obtém o usuário atual a partir do token JWT
    current_user = get_jwt_identity()

    # Consulta todos os pacientes no banco de dados
    pacientes = Paciente.query.all()  # Supondo que 'Usuario' seja a tabela que contém pacientes

    # Cria uma lista com os dados dos pacientes
    lista_pacientes = []
    for paciente in pacientes:
        lista_pacientes.append({
            'id': paciente.id,
            'nome': paciente.nome,  # Altere conforme o campo correto para o nome
        })
...
def listar_medicos():
    ...
    # Consulta todos os pacientes no banco de dados
    medicos = Medico.query.all()  # Supondo que 'Usuario' seja a tabela que contém pacientes
...
def listar_consultas():
    ...
    # Consulta todos os pacientes no banco de dados
    consultas = Consulta.query.all()  # Supondo que 'Usuario' seja a tabela que contém pacientes
...
def adicionar_paciente():
    ...
    try:
        ...
        return jsonify(error="Erro ao adicionar paciente!", details=str(e)), 500
...
def adicionar_medico():
    ...
    try:
        ...
        return jsonify(error="Erro ao adicionar médico", details=str(e)), 500
...
def detalhes_consulta(id):
    ...
    # Consulta a consulta com o ID fornecido
    consulta = Consulta.query.get(id)

    if not consulta:
        return jsonify(error="Consulta não encontrada."), 404

    # Retorna os detalhes da consulta
    consulta_data = {
        'id': consulta.id,
        'paciente': {
...
def deletar_consulta(id):
    ...
    try:
        ...
        return jsonify(error="Erro ao deletar consulta", details=str(e)), 500
...
def detalhes_medico(id):
    ...
    # Consulta o médico com o ID fornecido
    medico = Medico.query.get(id)

    if not medico:
        return jsonify(error="Médico não encontrado."), 404

    # Retorna os detalhes do médico
    medico_data = {
        'id': medico.id,
        'nome': medico.nome,
...
def deletar_medico(id):
    ...
    try:
        ...
        return jsonify(error="Erro ao deletar médico", details=str(e)), 500
...
def detalhes_paciente(id):
    ...
    # Consulta o paciente com o ID fornecido
...
@routes_bp.route('/paciente/<int:id>', methods=['DELETE'])
@jwt_required()  # Protege a rota com JWT
def deletar_paciente(id):
    ...
    try:
        ...
        return jsonify(error="Erro ao deletar paciente", details=str(e)), 500
